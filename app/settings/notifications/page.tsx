...
export default async function NotificationSettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <h1 className="text-2xl font-bold mb-6">{tr("Notifications")}</h1>
      <form action={updateNotificationPrefs} className="glass p-6 space-y-6 max-w-lg">
        <label className="flex items-center gap-3">
          ...
          />
          <span>
            <span className="font-semibold block">{tr("Email digests")}</span>
            <span className="text-xs text-muted">{tr("Challenge results, badge awards and moderation notices.")}</span>
          </span>
        </label>
        <p className="text-xs text-muted">
          {tr("In-app notifications for follows, badges and challenges are always on — they're how the galaxy talks to you.")}
        </p>

...
// The Copy tab of /admin/content. Was its own route; the page-level heading and
// lede now belong to the tab shell, so this file is only the form.
...
const GROUPS: { title: string; note?: string; keys: { key: string; label: string; long?: boolean }[] }[] = [
  ...
  // it is how a pricing page starts contradicting itself.
  {
    ...
    keys: [
      ...
      { key: "pricing.slotCount", label: "Video slots in the network rotation" },
      { key: "pricing.slotSeconds", label: "Seconds per video slot" },
  ...
  },
  {
    ...
    keys: [
      ...
      { key: "pricing.package.name", label: "Package name" },
      { key: "pricing.package.tagline", label: "Package tagline" },
      { key: "pricing.package.features", label: "What the package includes (one per line)", long: true },
...
export default async function CopyPanel() {

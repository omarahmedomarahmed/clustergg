...
import QuestGame, { type QuestRule, type QuestLogEntry } from "@/components/QuestGame";
...
export default function QuestMapHero({
  ...
  game?: { rules: QuestRule[]; log: QuestLogEntry[]; totalCp: number };
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-6xl px-4 pt-4 pb-8 md:pb-10">
        ...
        <div className="relative z-0 mx-auto w-full max-w-4xl aspect-[4/5] sm:aspect-[16/9] float-y">
          ...
          {game && <button aria-label={tr("Play quest")} onClick={() => openGame(null)} className="absolute inset-0 cursor-pointer" />}
          ...
          {tiers.map((t, i) => {
            const active = sel === i;
            return (
              <button key={t.id} onClick={() => (game ? openGame(i) : setSel(active ? null : i))}
          ...
          {game && (
            <button type="button" onClick={() => openGame(null)}
              className="absolute bottom-3 left-1/2 -translate-x-1/2 z-30 glow-btn pressable inline-flex items-center gap-2 rounded-full px-7 py-2.5 text-sm font-bold text-white shadow-xl">
      ...
      {game && play && (
        <QuestGame quest={q} holders={holders} rules={game.rules} log={game.log} totalCp={game.totalCp}

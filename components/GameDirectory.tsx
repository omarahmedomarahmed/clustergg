...
const KIND_LABEL: Record<string, string> = { champion: "Champions", hero: "Heroes", agent: "Agents", weapon: "Weapons" };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-2xl max-h-[88vh] overflow-hidden rounded-2xl border border-white/15 bg-[#04051a]" onClick={(e) => e.stopPropagation()}>
        <div className="relative h-44 sm:h-56">
          {(d?.splash || lite.image) && /* eslint-disable-next-line @next/next/no-img-element */
            <img src={d?.splash || lite.image} alt="" className={`absolute inset-0 h-full w-full ${lite.kind === "weapon" ? "object-contain p-6" : "object-cover"}`} />}
          <div className="absolute inset-0" style={{ background: "linear-gradient(to top, #04051a, rgba(4,5,26,0.2) 60%, transparent)" }} />
          <button onClick={onClose} className="absolute top-2 right-2 rounded-full bg-black/50 p-1.5 text-white hover:bg-black/70"><Icon name="x" size={16} /></button>

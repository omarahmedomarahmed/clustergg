...
type RegionRow = { region: string | null; name: string; slug: string; avatar?: string | null; ign?: string | null; providerData?: Record<string, unknown> | null; accountId?: string; provider?: string; country?: string | null };
...
export function computeRealRegions(rows: RegionRow[], pins: PinMap, fallbackCode?: string): RegionStat[] {
  ...
  for (const a of rows) {
    ...
    if (e.gamers.length < 8 && !e.gamers.some((x) => x.slug === a.slug)) {
      e.gamers.push({ name: a.name, slug: a.slug, avatar: gameAvatarOf(a.providerData) ?? a.avatar ?? null, ign: a.ign ?? undefined, accountId: a.accountId, provider: a.provider, country: a.country ?? null });
    }
  }

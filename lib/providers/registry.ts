...
export type MetricDef = {
  ...
  /**
   * Whether a challenge may be *scored* on this metric.
   *
   * ===== ADDED ON ARRIVAL. docs/00-TRUTH.md §4.3, C16 =====
   *
   * "Wins and matches, counted. **No win rate. No percentages. No ratios.**"
   *
   * A ratio cannot be scored the way this platform scores, and the reason is
   * structural rather than a matter of taste. Scoring is `Δmetric` from a
   * per-participant baseline: the delta of a *count* is what happened this
   * week, and the delta of a *ratio* is nothing at all. A player who goes from
   * 60% to 61% has a delta of 1 whether that took four hundred games or four,
   * and a player who starts at 90% and holds it scores zero for a perfect
   * week.
   *
   * So percentages and ratios stay in the registry — a challenge page may
   * still *show* a win rate, and gamers want to see it — but they are not
   * offered in the builder and cannot decide money. `false` here, and a test
   * walks the whole registry asserting that everything shaped like a ratio
   * carries it.
   */
  scoreable?: boolean;
  /**
...
export type ProviderDef = {
  ...
  /**
   * Not sellable, whatever the environment says.
   *
   * ===== ADDED ON ARRIVAL. docs/11-PORTED-CODE.md =====
   *
   * `isProviderLive` answers "are this provider's keys configured?". For
   * VALORANT that question has the wrong answer: `RIOT_API_KEY` is set,
   * because League needs it, so VALORANT reads as live — and it is not. The
   * personal key has **no VAL method at all**, not even platform status, so
   * VALORANT cannot be scored, rank-gated or health-checked. A challenge on
   * it is unsellable until Riot grants production access.
   *
   * `identityOnly` was already there and is a different statement: it keeps
   * VALORANT out of the *link* pickers. This keeps it out of everything a
   * key would otherwise switch on. Both are true of VALORANT and neither
   * implies the other.
   */
  notLive?: string;
...
export const PROVIDERS: ProviderDef[] = [
  ...
  },
  {
    ...
    capabilities: [
      ...
      { key: "win_rate", label: "Win rate %", unit: "%", higherIsBetter: true, scoreable: false },
  ...
  },
  {
    ...
    capabilities: [
      ...
      // ===== ADDED ON ARRIVAL. docs/11-PORTED-CODE.md =====
      //
      // `matches = wins + losses`, from the same single call. C19: matches
      // played is always counted and always shown, whether or not it is
      // scored — and C20 says both numbers come from one request, which is
      // why this is a derived capability rather than a second endpoint.
      { key: "matches", label: "Ranked matches", higherIsBetter: true },
      { key: "win_rate", label: "Win rate %", unit: "%", higherIsBetter: true, scoreable: false },
  ...
  },
  {
    ...
    notLive: "The Riot personal key has no VALORANT endpoint — not ranked, not content, not even platform status. VALORANT cannot be sold until Riot grants production access.",
    capabilities: [
  ...
  },
  {
    ...
    capabilities: [
      ...
      { key: "kd_ratio", label: "K/D ratio", higherIsBetter: true, scoreable: false },
  ...
  },
  {
    ...
    capabilities: [
      ...
      { key: "accuracy", label: "Hit accuracy %", unit: "%", higherIsBetter: true, scoreable: false },
  ...
  // ===== Phase 3 — flagged, not built (legal risk per plan §15) =====
  {
    ...
    notLive: "No official public API. Unofficial wrappers risk account suspension, so this needs legal sign-off before it can be enabled.",
    capabilities: [],
  },
  {
    ...
    notLive: "Unofficial API only. This needs legal sign-off before it can be enabled.",
    capabilities: [],
  },
];
...
export function isProviderLive(p: ProviderDef): boolean {
  // A provider declared not live is not live, whatever keys happen to be set.
  // This is checked FIRST on purpose: the case it exists for — VALORANT — is
  // one where the key check below would say yes.
  // `notLive` now carries every reason a provider cannot be live, including
  // the two that were a hard-coded pair of ids here: PlayStation Network and
  // Call of Duty, both blocked pending legal sign-off. A rule written as a
  // list of names only ever covers the names somebody remembered, and it
  // cannot be tested as a property — which is how this one was found.
  if (p.notLive) return false;
...
// ===== Scoreable metrics, and the queue a League challenge reads =====
//
// Both added on arrival for the new model. docs/00-TRUTH.md §4.3 and §4.4.

/** The shapes that can never be scored, whatever a registry entry claims. */
const RATIO_SHAPED = /(_rate|_ratio|^kda$|^accuracy$)/;

/**
 * Whether a metric may decide money.
 *
 * Two conditions, and the second is the one that matters: a metric is
 * scoreable only if it does not *look* like a ratio. An explicit
 * `scoreable: false` is a statement somebody made; the pattern is what catches
 * the metric added next year by somebody who never read §4.3. A test walks the
 * registry and fails if the two ever disagree, so neither can drift.
 */
export function isScoreable(m: MetricDef): boolean {
  if (m.scoreable === false) return false;
  if (m.unit === "%") return false;
  return !RATIO_SHAPED.test(m.key);
}

/** What the challenge builder is allowed to offer for a provider. */
export function scoreableMetrics(providerId: string): MetricDef[] {
  return (getProvider(providerId)?.capabilities ?? []).filter(isScoreable);
}

/**
 * Which League queue a challenge scores.
 *
 * C24: selectable per challenge — solo/duo, flex, or both. The ported adapter
 * read solo queue only and hard-coded it, which made every challenge a solo
 * challenge and gave a flex-only player a permanent zero.
 *
 * `both` sums the two queues. That is a decision worth naming: a game played
 * in flex is a game played, and `points = Δwins × 10 + Δmatches` counts games.
 */
export const QUEUES = ["solo", "flex", "both"] as const;
export type Queue = (typeof QUEUES)[number];
export const DEFAULT_QUEUE: Queue = "solo";

export function isQueue(v: unknown): v is Queue {
  return QUEUES.includes(v as Queue);
}

// ===== Ownership proof, per provider =====
//
// G4: proof is required where the game's API supports it. G5: where it cannot,
// entry is allowed with an unproven account — **no badge, no warning, no
// second class.** It is not the gamer's fault the publisher has no endpoint,
// and a UI that marks them out punishes them for it.
//
// So this answers one question — *can this provider prove ownership at all?* —
// and every caller reads it rather than guessing from the provider id.

/** How a provider can prove the person in front of us owns the account. */
export type ProofMethod = "icon" | "oauth" | "openid" | "code";

/**
 * Derived from what the provider actually offers, not from a hand-kept list.
 *
 * A list would need editing every time a provider is added, and the one
 * somebody forgets silently becomes "cannot prove" — which fails open, in the
 * direction that lets an unproven account look proven later.
 */
export function proofMethodFor(providerId: string): ProofMethod | null {
  const p = getProvider(providerId);
  if (!p) return null;
  // Riot lets a gamer set their profile icon to a value we name, then reads
  // it back. That is real proof of control of the account.
  if (p.id.startsWith("riot-")) return "icon";
  // An in-game verification code, delivered to the account's own mailbox.
  if (p.authType === "vc" || p.linkFlow === "vc") return "code";
  if (p.authType === "oauth") return "oauth";
  if (p.authType === "openid") return "openid";
  // Everything else is a public API keyed by a name anybody can type. The
  // account can be shown to EXIST. It cannot be shown to be theirs.
  return null;
}

/** True when a gamer can be asked to prove this account, and refused if they don't. */
export function canProveOwnership(providerId: string): boolean {
  return proofMethodFor(providerId) !== null;
}

...
export type AccountRef = {
  ...
  /**
   * Which League queue to read. Added on arrival — C24.
   *
   * The ported adapter read `RANKED_SOLO_5x5` and nothing else, so a flex-only
   * player synced zero wins forever and a flex challenge could not exist. The
   * queue belongs to the challenge, not to the account, so it arrives with the
   * request.
   */
  queue?: "solo" | "flex" | "both";
  /**
   * Whether this call may spend the tight endpoint budget.
   *
   * `summoner-v4 by-puuid` is capped at **1,600 requests a minute** — seventy
   * times tighter than its neighbours, and docs/11-PORTED-CODE.md says in as
   * many words: never put it on a hot path, display names only. The ported
   * adapter called it on *every* stat sync, which is the hot path by
   * definition: one call per linked League account per hour.
   *
   * So the hourly sync leaves it alone and reads `league-v4` only — which
   * returns tier, division, LP, wins and losses for both queues in one request
   * at 20,000 per ten seconds, and is the entire scoring and rank-gating
   * requirement. Linking and reconnecting pass `rich: true`, because those are
   * once per account and want the profile icon and summoner level.
   */
  rich?: boolean;
};
...
const opendota = {
  ...
  async fetchStats(a: AccountRef): Promise<StatsResult> {
    try {
      ...
      const { dotaRankLabel } = await import("./registry");
...
const riotLol = {
  ...
  async fetchStats(a: AccountRef): Promise<StatsResult> {
    ...
    try {
      ...
      // ===== league-v4 ALWAYS. summoner-v4 ONLY WHEN ASKED =====
      //
      // `league/v4/entries/by-puuid` returns tier, division, LP, wins and
      // losses for BOTH queues in one request, at 20,000 per ten seconds. That
      // single call is the whole scoring and rank-gating requirement.
      //
      // `summoner-v4 by-puuid` is 1,600 a minute — the one tight endpoint on
      // the key — and it only carries the summoner level and the profile icon.
      // It used to be called on every sync alongside the line above. Now it is
      // `rich` only: linking, reconnecting, verifying. Champion mastery rides
      // with it for the same reason, and both stay isolated so a hiccup in
      // either never fails the rank sync.
      const entries = await j<any[]>(
        `https://${platform}.api.riotgames.com/lol/league/v4/entries/by-puuid/${a.providerAccountId}`,
        { headers },
      );
      const [summoner, masteryRaw] = a.rich
        ? await Promise.all([
            j<any>(`https://${platform}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${a.providerAccountId}`, { headers }).catch(() => null),
      ...
      // the same value.
      const { lolTierValue } = await import("./registry");
      const tierVal = (e: any) =>
      ...
      // ===== THE QUEUE IS THE CHALLENGE'S, AND `matches` IS BOTH NUMBERS =====
      //
      // C20: wins and losses come from this one call, and `matches = wins +
      // losses`. C16 rules out the ratio, so `win_rate` is no longer emitted —
      // it is a registry capability a page may show, not something a sync
      // writes into a scoreable series.
      const queue = a.queue ?? "solo";
      const counted = queue === "flex" ? [flex] : queue === "both" ? [solo, flex] : [solo];
      const sum = (pick: (e: any) => number | undefined) =>
        counted.reduce<number | undefined>((acc, e) => {
          const v = e ? pick(e) : undefined;
          return v == null ? acc : (acc ?? 0) + v;
        }, undefined);
      const wins = sum((e) => num(e?.wins));
      const losses = sum((e) => num(e?.losses));
      const matches = wins == null && losses == null ? undefined : (wins ?? 0) + (losses ?? 0);
      ...
      try {
        const { lolPersistBits } = await import("./riot-lol-rich");
...
const mobileLegends = {
  ...
  async fetchStats(a: AccountRef): Promise<StatsResult> {
    const { decryptSecret } = await import("../core/crypto.ts");
    const { fetchMlbbStats } = await import("./mlbb");

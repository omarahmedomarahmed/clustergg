...
import { attributionSentence, attributionShort } from "../identity/attribution.ts";
 ...
 * the same reason.
 */
function ruleWordings(): { phrases: string[]; what: string; use: string }[] {
  const generated = `${attributionSentence()} ${attributionShort()}`.toLowerCase();
  const fromModule = ["half an entrant", "a whole entrant"].filter((p) =>
    generated.includes(p),
  );
  return [
    {
      // The deleted model's wordings sit beside the live ones: both are the
      // rule stated in words, and the deleted ones are worse, because they are
      // the rule as it used to be.
      phrases: [...fromModule, "worth half", "half to each", "½ to each", "parent server, credited"],

...
/**
 * The checkout session a brand is sent to.
 *
 * Returns the URL, or null when Stripe is not configured — which is the demo,
 * where the builder pays the invoice directly. Fenced rather than throwing:
 * house rule 11 applied to a purchase path, and a deployment without keys
 * should show a builder that says so rather than a page that crashes.
 */
export async function createCheckoutSession(input: {
  ...
  const body = new URLSearchParams({
    mode: "payment",
    // The key that makes the webhook idempotent-by-invoice rather than
    // guessing from an amount.
  ...
  const response = await fetch("https://api.stripe.com/v1/checkout/sessions", {
    ...
    headers: {
      ...
      // Stripe's own idempotency, on the way out. A double-clicked Confirm &
      // Pay creates one session rather than two.
      "idempotency-key": `invoice-${input.invoiceId}`,
    },
    body,
  });

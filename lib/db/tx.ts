...
//
// PORTED AS-IS from the previous platform (docs/11-PORTED-CODE.md), and the
// narrative below is the history that produced it. The three callers it names —
// quest awards, trophy purchase, CP wallets — no longer exist; they were
// deleted by ruling (docs/00-TRUTH.md §11). The finding survives them, because
// it is about the driver and not about those features: this platform's money
// paths are redemptions, vault splits, pool allocations and trophy assignment,
// and every one of them is a read-then-write that needs a real transaction.
//

...
| B72–B85 | **the remaining owner decisions: age, onboarding, ad views, card parity** | **All answered; `docs/PLAN.md` carries the full build.** **Ad views simplified to the honest floor:** every card the bot draws is ONE view, public or private alike. The 5%-of-members estimate for public posts is dropped entirely — a modelled number labelled as delivery is the fabricated ROAS in better arithmetic, so there is now no estimate anywhere on a brand report and no `views` or `estimated` column to get wrong. The brand is told which SERVER a card came from, never whether it was public or private; we store that only to prove our own numbers. **This makes our sellable inventory much smaller and B78 restates the model on it** — understating is the safe side of that line. **Age:** bands are **Under 13 / 13–17 / 18+** (thirteen, not fourteen — COPPA's line, and the lawyer confirms the wording). Under-13 is READ-ONLY: no linking, no challenges, no CP, no trophies. 13–17 earns but cannot redeem. Asked as the first thing on sign-in, one click, no confirm; a popup on every gamer page until answered, then every five navigations until unlocked. **No CP accrues before a band is set and there is no backfill**, though the actions are still logged. **Onboarding is TWO steps, not five** — link a game account, customize the profile — and **a flag set from Discord counts as customization**, so the whole path completes without ever leaving Discord. Sharing the card is removed as a gate. Locked CP caps at **5,000** and can be neither spent nor redeemed; **existing gamers are grandfathered and nothing already earned is ever locked.** The profile editor gains a **real Satori render** of the bot card, debounced — not an HTML lookalike, because a preview that differs from the real thing is a lie that looks like a feature — which needs its own line in B46's render budget. Filed: **B81** (view logging), **B82** (the report), **B83** (onboarding), **B84** (the visual gamer guide), **B85** (card parity + the admin layout editor). |
| ALL | **twelve stored card layouts silently undid the redesign** | **Found from production screenshots, confirmed by reading the database, fixed by versioning.** A challenge card went to fifteen servers with its content crammed into 59.5% of the width, the gradient bar back on and the watermark at 30% opacity in the corner. `parseLayout` merged stored values over the new defaults FIELD BY FIELD, and every one of the twelve stored layouts predated B56.0. **The near miss that should have caught it is the worst part:** `layout.ts` already carried a comment saying stored layouts predate the redesign — written when `bar` was defaulted off. The staleness was spotted for ONE field and the obvious next question, *what about `content`, `mark`, `badge`, `plate`?*, was never asked. Fixed with `LAYOUT_VERSION = 2`: a stored layout without it is **discarded, not migrated**, because the old numbers describe a frame that no longer exists. No production write — it recovers by deploying, and the old JSON stays in the row. **Also found while confirming it: the planet card cannot draw its globe** — `lib/cards/data.ts:203` selects `planetBgUrl` and never `planetImageUrl`, so the art was never fetched, which is why the card only ever showed the space behind the planet. |
| — | *(next amendment here)* | |

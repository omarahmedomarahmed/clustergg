...
| B54 | **PARTIAL — the mechanical half is done, the redesign is not** | Shipped: the in-game name leads every standings row on BOTH cards (found a second violation while asserting the first — `challengeStandingsCard` read `<cluster> · <account>`, the Cluster name first, and my assertion pointed at the wrong field which is how I found it); the last fixed-height text box removed, with a test that fails on any `height:` beside a `fontSize:` so the descender bug cannot come back; and every card kind asserted to render through Satori without throwing. **Still owed, and it is the larger half:** the fixed top strip with branding right and identity left, background images on challenge and planet cards, the semi-transparent game logo and mascot in the strip, and designing each card around its body rather than a shared template. That is a visual redesign of thirteen card kinds and it needs an eye on the output, not a test — so **B3, B13, B14, B20, B27 and B28 are still blocked behind it** exactly as B54.0 says. |
| — | *(next amendment here)* | |
...
| `tests/db/cards.mts` | **B54** | no text box carries a fixed height; every standings row leads with the in-game name on BOTH cards; Satori renders every kind without throwing | ☑ 22 (`tests/ui/cards.mjs` still owed with the redesign) |
